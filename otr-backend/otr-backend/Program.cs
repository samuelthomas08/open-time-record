...
// Explicit persistence so the key ring (used to encrypt/decrypt the stored SMTP
// password) survives container restarts instead of regenerating and orphaning it.
...
static async Task SeedSuperadminRoleAsync(IServiceProvider services)
{
    ...
    // Applies any pending EF Core migrations so a fresh (or upgraded) deployment
    // never needs a manual `dotnet ef database update` step.
