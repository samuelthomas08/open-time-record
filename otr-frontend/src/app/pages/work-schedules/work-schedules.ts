...
export class WorkSchedules implements OnInit {
  ...
  protected manage(user: User): void {
    this.dialogService.create({
      zTitle: `Arbeitszeit — ${user.firstName} ${user.lastName}`,
      zContent: ManageWorkScheduleDialog,
      zHideFooter: true,
      zWidth: '30rem',
      zData: { user, schedules: this.schedulesFor(user), onSuccess: () => this.load() },
    });
  }
  ...
  private async load(): Promise<void> {
    ...
    try {
      const [managedUsers, schedules, mySchedules] = await Promise.all([
